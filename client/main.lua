...
local function IsNitroInstalled(vehicle)
    return vehiclesInstalled[vehicle]
end

local function setNitroInstalled(vehicle, enable)
    vehiclesInstalled[vehicle] = enable or false
    return
end

local function GetNitroInstalled(vehicle)
    if vehiclesInstalled[vehicle] ~= nil then
        print("FOUND")
        return vehiclesInstalled[vehicle]
    end
    return false
...
exports("GetNitroInstalled", GetNitroInstalled)

RegisterNetEvent('sw-nitro:GetNitroInstalled')
AddEventHandler('sw-nitro:GetNitroInstalled', function(vehicle)
    if vehiclesInstalled[vehicle] ~= nil then
        print("FOUND")
        return vehiclesInstalled[vehicle]
    end
...
RegisterNetEvent('sw-nitro:SetNitroInstalled')
AddEventHandler('sw-nitro:SetNitroInstalled', function(vehicle, enable)
    vehiclesInstalled[vehicle] = enable
end)
RegisterNetEvent('sw-nitro:InstallNitro')
AddEventHandler('sw-nitro:InstallNitro', function(source, enable)
    if enable == nil then
        enable = false
    end
    print("ENABLE ", enable)
...
local function NitroLoop(lastVehicle)
    ...
    if Config.TurboRequired and not IsToggleModOn(vehicle, 18) then
        return 0
    end
    ...
    -- local isInstalled = IsNitroInstalled(vehicle) or false

    -- if not isInstalled then
    --     setNitroInstalled(vehicle, false)
    --     print("no")
    --     return
    -- else
    --     setNitroInstalled(vehicle, true)
    -- end
    if isRunning and isEnabled and isFueled then
        if isDriving then
            if not isBoosting and not isPurging then
